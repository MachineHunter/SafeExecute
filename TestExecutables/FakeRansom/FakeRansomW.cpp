...
int main(int argc, char *argv[]) {
	if(argc!=2) {
		cout << "Usage:" << endl << ".\\FakeRansomW.exe <directory-to-encrypt>" << endl;
		return 1;
	}
		...
		do {
			if((ffd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) == 0) {
				if(strstr(ffd.cFileName, ".txt")) {
					...
					DWORD writesize;
					char buf[] = "\nencrypted\n";
					wchar_t filepathW[MAX_PATH*2];
					size_t retsize;
					mbstowcs_s(&retsize, filepathW, MAX_PATH*2, (const char*)filepath, _TRUNCATE);
					hFile = CreateFileW(filepathW, GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
